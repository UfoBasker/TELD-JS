...
(function () {
    ...
    MarkerManager.prototype.addMapGPSPoints = function (posInfo) {
        var arr1 = GPS.gcj_encrypt(posInfo.gpsPos.Latitude, posInfo.gpsPos.Longitude);
     ...
     * 刷新标志
     */
    MarkerManager.prototype.refreshMarker = function (autoShow) {
      var me = this;
      var needAdd = false;
      for (var i = 0; i < this._posLength; i++) {
           ...
           marker.addInfo = this._numPositions[i].addInfo;
    ...
    /**
         * 显示标志
         */
    MarkerManager.prototype.showMarker = function (mark) {
       if (typeof(mark.displayFlg) != "undefined") {
           if (typeof(mark.isAdded) == "undefined" || mark.isAdded == false) {
               map.addOverlay(mark);
            } else {
               mark.show();
            }
           mark.isAdded = true;
           mark.displayFlg = true;
        }
    }
